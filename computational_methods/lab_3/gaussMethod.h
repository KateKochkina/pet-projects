...
int gaussHessenberg(const MyType* const* const H, const MyType* const b, MyType* const x,
                    const int n)  // Метода Гаусса
{
    ...
    for (int k = 0; k < n; ++k) {
        ...
        // Прямой ход
        MyType t = A[k + 1][k] / A[k][k];
        for (int j = k; j < n; ++j) {
            A[k + 1][j] -= A[k][j] * t;
        }
        b_copy[k + 1] -= b_copy[k] * t;
    }

    backward(A, b_copy, x, n);
