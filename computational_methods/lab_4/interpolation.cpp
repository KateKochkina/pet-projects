...
double Func(double x) {
  ...
  return pow((4 * x * x * x + 2 * x * x - 4 * x + 2), sqrt(2)) +
      asin(1 / (5 + x - x * x)) - 5;
  ...
  // return sinh((sqrt(13)*x*x*x - 9*x - 5 - sqrt(17)) / 10) +
  //     tan((x*x + x + pow(2, 1/3)) / (3*x - 5)) + 0.6;
...
int main() {
    char stopCh;           // Выход из программы
    ...
    do {
        ...
        // std::cout << "L(2.2) = " << LagrangePolynomial(2.2, xValues, fValues) << '\n';
		// std::cout << "exp(2.2) = " << exp(2.2) << '\n';
		// std::cout << "diff = " << LagrangePolynomial(2.2, xValues, fValues) - exp(2.2) -  << '\n';
        std::cout << "Continue? ";
        std::cin >> stopCh;
    } while (stopCh != 'n');
